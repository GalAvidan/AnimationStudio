...
- Confirm `Vault/hub/studios/AnimationStudio/archive/projects/<slug>/` does not already exist. If it does, stop and report the archive collision — do not overwrite.

## Steps

1. Read `{projects}/<slug>/project.config.ts` to confirm the project name, adapter, and variants.
2. Move the entire `{projects}/<slug>/` tree to `Vault/hub/studios/AnimationStudio/archive/projects/<slug>/`.
3. In `Vault/hub/studios/AnimationStudio/archive/archive-index.md`, append a row for the archived project:
   - Columns: slug, adapter, variants (comma-separated), archive date (YYYY-MM-DD), notes.
4. Create `Vault/hub/studios/AnimationStudio/archive/projects/<slug>/manifest/rehydration-map.md` documenting:
   - Original path: `{projects}/<slug>/`
   - Archive path: `Vault/hub/studios/AnimationStudio/archive/projects/<slug>/`
   - Adapter and variants at time of archive.
   - Any outstanding TODOs from `project.config.ts` or spec files.
5. If `Vault/hub/studios/AnimationStudio/archive/projects/<slug>/decisions.md` exists, fold unresolved or reversible decisions into `manifest/rehydration-map.md` under "Open decisions to revisit".
6. If `Vault/hub/studios/AnimationStudio/archive/projects/<slug>/notes.md` exists, promote any entries marked `reusable: yes` into `agent-context/patterns/index.md` with category hints (`motion`, `scene`, or `audio`).
7. Before reporting completion, ensure status context is captured:
   - If `Vault/hub/studios/AnimationStudio/archive/projects/<slug>/status.md` exists, append a final history line with phase `done/archived`.
...
## Output
Project tree moved to `Vault/hub/studios/AnimationStudio/archive/projects/<slug>/`, archive-index updated, `rehydration-map.md` created.
